...
int main(int argc, char* argv[]){
    ...
    double rin = 2.0 * pr;
    double rout = 5.0 * pr;//0.2 * pa * pow((pm / (3 * star.m)), (1./3.)); //hill radius

    const double theta_p = 45.0 * M_PI / 180.;
    int Ntest = 5;
    ...
    //rebx_set_param_int(rebx, &sim->particles[1].ap, "primary", 1);

    // Planetary Spin alignment torque
    struct reb_vec3d alignment_vec = reb_tools_spherical_to_xyz(1., 45. * M_PI / 180., 0.);
    const double alignment_ts = 1e3 * 2 * M_PI;
    //rebx_set_param_vec3d(rebx, &sim->particles[1].ap, "alignment", alignment_vec);
    //rebx_set_param_double(rebx, &sim->particles[1].ap, "alignment_ts", alignment_ts);
